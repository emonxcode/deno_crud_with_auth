...
export default {
  ...
  findByEmail: async (email: any) => {
    const result = await dbClient.query("select * from users where email = ?", [
      email,
    ]);
    return result;
  },
  ...
  updateByID: async (data: User) => {
    const getData = await dbClient.query("select * from users where id", [
      data.id,
    ]);
    if (getData != null) {
      await dbClient.query(
        `update users set name = '${data.name}', email = ${data.email} where id = ${data.id}`,
      );
      return null;
    } else {
